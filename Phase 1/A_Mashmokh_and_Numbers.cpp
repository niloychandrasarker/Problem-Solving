...
vector<int> mashmokh_and_numbers(int n, int k) {
    // Minimum number of points required
    int min_points = n / 2;

    // Check if it's possible to achieve k points
    if (k < min_points) {
        return {-1};
    }
    ...
    // Start with the first two elements to ensure minimum k points
    int x = 1;
    result.push_back(x);
    result.push_back(2 * x);

    // Adjust k by subtracting the initial points we have guaranteed
    k -= 1;

    // Fill the rest with consecutive integers
    int current_value = 3;
    for (int i = 2; i < n; ++i) {
        // Ensure the number is distinct
        while (find(result.begin(), result.end(), current_value) != result.end()) {
            ++current_value;
        }
...
int main() {
    ...
    vector<int> result = mashmokh_and_numbers(n, k);
