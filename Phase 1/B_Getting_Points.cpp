...
long long max_rest_days(long long n, long long p, long long l, long long t) {
    long long cntTasks = (n + 6) / 7;

    long long lf = 0LL;
    long long rg = n;
    while (rg - lf > 1) {
        long long mid = (lf + rg) / 2;
        if (calc(mid, l, t, cntTasks) >= p)
            rg = mid;
        else
            lf = mid;
    }
...
int main() {
    ...
    while (tc--) {
        long long n, p, l, t;
        cin >> n >> p >> l >> t;
        cout << max_rest_days(n, p, l, t) << endl;
    }
